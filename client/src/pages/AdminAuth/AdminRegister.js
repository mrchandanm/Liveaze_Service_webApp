...
import ProfessionList from '../../components/DropdownLists/ProfessionList';
const AdminRegister = () => {
    const options=[
        {label:"Plumber", value:"Plumber"},
        {label:"Electrician", value:"Electrician"},
        {label:"Carpenter", value:"Carpenter"},
        {label:"Civil Worker", value:"Civil Worker"},
    ...
    const handleSubmit=async(e)=>{
        ...
        if(tnc){
        try {
            const res=await axios.post(`${process.env.REACT_APP_API}/api/v1/auth/admin-register`,{name,email,password,phone,address1,address2,profession,pin,city,state,answer});
            if(res && res.data.success){
                 navigate('/admin-login');
                toast.custom(<Success title="Register as admin Successful"/>);
            }
            else{
...
<div className='col-12'>
  <ProfessionList profession={profession} setProfession={setProfession}/>
</div>
  <div className="col-12">
